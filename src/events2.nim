...
proc checkForOpenSpaceEvents(roll: Positive): bool {.raises: [KeyError, IOError,
    ValueError, Exception], tags: [WriteIOEffect, RootEffect], contractual.} =
  ## Check of events in open space
  ##
  ## * roll - the random roll for check for the event
  ##
  ...
  of 6..20:
    ...
    if pilotIndex > 0:
      addMessage(message = "Sudden bad weather causes your travel to take longer.",
          mType = otherMessage, color = red)
      var timePassed: range[-1_000..100] = 60 - getSkillLevel(
          member = playerShip.crew[pilotIndex], skillIndex = pilotingSkill)
      if timePassed < 1:
        timePassed = 1
...
proc checkForEvent*(): bool {.raises: [ValueError, IOError,
  ...
  if baseIndex == 0:
    return checkForOpenSpaceEvents(roll = roll)
  ...
  if playerShip.speed == docked:
    ...
    # Lost cargo in the base
      ...
      addMessage(message = "During checking your ship's cargo, you noticed that you lost " &
          ...
          ".", mType = otherMessage, color = red)
      let item: InventoryData = playerShip.cargo[roll2]
      updateCargo(ship = playerShip, amount = 0 - lostCargo, cargoIndex = roll2,
