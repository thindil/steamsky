...
try:
  ...
  writeLine(stderr, "Failed to count days difference between two dates.")

echo "Testing generateRoboticName."
try:
  assert generateRoboticName().len() > 0
except AssertionDefect:
  writeLine(stderr, "Failed to generate a robotic name.")
...
for i in 1..5:
  try:
    ...
    writeLine(stderr, "Failed to generate a random number between 1 and 5")
try:
  assert getrandom(5, 5) == 5
except AssertionDefect:
  writeLine(stderr, "Failed to generate a random number between 5 and 5")
