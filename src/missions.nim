  ...
  body:
    ...
    if failed:
      ...
      if rewardAmount > 0:
        ...
        updateCargo(ship = playerShip, protoIndex = moneyIndex,
            amount = rewardAmount, quality = normal, craftBonus = none,
            craftMalus = none)
    ...
    if mission.mType == deliver:
      updateCargo(ship = playerShip, protoIndex = mission.itemIndex,
          amount = -1, quality = normal, craftBonus = none, craftMalus = none)
...
proc generateMissions*() {.raises: [KeyError], tags: [],
  ...
  while index > 0:
    ...
    case mType
    of deliver:
      mission = initMissionData(mtype = deliver, time = 1, targetX = 0,
          targetY = 0, reward = 1, startBase = 1, finished = false,
          itemIndex = missionsItems[
          getRandom(min = 0, max = missionsItems.high)], multiplier = 1.0)
    of destroy:
      mission = initMissionData(mtype = destroy, time = 1, targetX = 0,
          targetY = 0, reward = 1, startBase = 1, finished = false,
          shipIndex = enemies[
      ...
      while true:
        ...
        if skyMap[missionX][missionY].baseIndex == 0 and missionX !=
          ...
          break
    of patrol:
      mission = initMissionData(mtype = patrol, time = 1, targetX = 0,
          targetY = 0, reward = 1, startBase = 1, finished = false,
      ...
      if mission.target == 1:
        continue
    of explore:
      mission = initMissionData(mtype = explore, time = 1, targetX = 0,
          targetY = 0, reward = 1, startBase = 1, finished = false,
              multiplier = 1.0, target = 1)
      for j in 1..10:
        missionX = getRandom(min = minX, max = maxX)
      ...
      if mission.target == 1:
        continue
    of passenger:
      mission = initMissionData(mtype = passenger, time = 1, targetX = 0,
          targetY = 0, reward = 1, startBase = 1, finished = false,
          multiplier = 1.0,
