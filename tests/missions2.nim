...
suite "Unit tests for missions2 module":
  ...
  playerShip.crew.add(initMemberData(morale = [1: 50.Natural, 2: 0.Natural],
      ...
      1, 1, 0, 0], order = pilot, loyalty = 100, health = 100, tired = 0,
      hunger = 0,
      thirst = 0, skills = @[initSkillInfo(index = 4, level = 4,
          experience = 0)],
  ...
  playerShip.modules.add(y = initModuleData(mType = ModuleType2.engine,
      protoIndex = 3, durability = 100, fuelUsage = 4, power = 2000,
      disabled = false,
      maxDurability = 100, name = "Engine", weight = 1))
  playerShip.modules.add(y = initModuleData(mType = ModuleType2.cockpit,
      protoIndex = 5, durability = 100, maxDurability = 100, name = "Cockpit", weight = 1))
  playerShip.modules.add(y = initModuleData(mType = ModuleType2.cabin,
      protoIndex = 4, durability = 100, owner = @[0], maxDurability = 100,
          name = "Cabin", weight = 1))
  playerShip.modules.add(y = initModuleData(mType = ModuleType2.cabin,
      protoIndex = 4, durability = 100, owner = @[1], maxDurability = 100,
          name = "Cabin", weight = 1))
  playerShip.cargo = @[]
  ...
  skyBases[1] = initBaseRecord(skyX = 1, skyY = 1, population = 100,
      baseType = "1", owner = "POLEIS")
  ...
  test "Finish an accepted mission":
    ...
    missions.acceptedMissions.add(y = initMissionData(mType = explore,
        time = 1000, targetX = 2, targetY = 2, reward = 1, startBase = 1,
            finished = true, multiplier = 1.0, target = 0))
    finishMission(0)
    check:
  ...
  test "Finish an accepted passenger mission":
    playerShip.crew.add(initMemberData(morale = [1: 50.Natural, 2: 0.Natural],
        homeBase = 1, faction = "DRONES", orders = [0.Natural, 0, 0, 1, 1, 1, 2,
        1, 1,
        1, 0, 0], loyalty = 100, health = 100, tired = 0, hunger = 0,
        thirst = 0, skills = @[], attributes = @[attribute, attribute,
            attribute, attribute], contractLength = 1000))
    missions.acceptedMissions.add(y = initMissionData(mType = passenger,
            time = 1000, targetX = 1, targetY = 1, reward = 1, startBase = 1,
            finished = false, multiplier = 1.0, data = 2))
    finishMission(0)
    check:
      acceptedMissions.len == 0
