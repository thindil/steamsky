      ...
      procedure Load_Selected_Data(Data_Name, File_Name: String) is
         ...
         procedure Load_Data_File(Local_Data_Name: String) is
            ...
            begin
               ...
               Item_Index := 0;
               Fill_Bases_Syllables_Start_Loop :
               loop
                  Syllable :=
                    To_Bounded_String
                      (Source =>
                         Value
                           (Item =>
                              Get_Ada_List_Value
                                (L_Index => 1, I_Index => Item_Index)));
                  exit Fill_Bases_Syllables_Start_Loop when Length
                      (Source => Syllable) =
                    0;
                  SyllableString_Container.Append
                    (Container => Base_Syllables_Start, New_Item => Syllable);
                  Item_Index := Item_Index + 1;
               end loop Fill_Bases_Syllables_Start_Loop;
               Item_Index := 0;
               Fill_Bases_Syllables_End_Loop :
               loop
                  Syllable :=
                    To_Bounded_String
                      (Source =>
                         Value
                           (Item =>
                              Get_Ada_List_Value
                                (L_Index => 2, I_Index => Item_Index)));
                  exit Fill_Bases_Syllables_End_Loop when Length
                      (Source => Syllable) =
                    0;
                  SyllableString_Container.Append
                    (Container => Base_Syllables_End, New_Item => Syllable);
                  Item_Index := Item_Index + 1;
               end loop Fill_Bases_Syllables_End_Loop;
               Item_Index := 0;
               Fill_Bases_Syllables_Post_Loop :
               loop
                  Syllable :=
                    To_Bounded_String
                      (Source =>
                         Value
                           (Item =>
                              Get_Ada_List_Value
                                (L_Index => 3, I_Index => Item_Index)));
                  exit Fill_Bases_Syllables_Post_Loop when Length
                      (Source => Syllable) =
                    0;
                  SyllableString_Container.Append
                    (Container => Base_Syllables_Post, New_Item => Syllable);
                  Item_Index := Item_Index + 1;
               end loop Fill_Bases_Syllables_Post_Loop;
               Item_Index := 0;
               Fill_Male_Syllables_Start_Loop :
               loop
                  Syllable :=
                    To_Bounded_String
                      (Source =>
                         Value
                           (Item =>
                              Get_Ada_List_Value
                                (L_Index => 4, I_Index => Item_Index)));
                  exit Fill_Male_Syllables_Start_Loop when Length
                      (Source => Syllable) =
                    0;
                  SyllableString_Container.Append
                    (Container => Male_Syllables_Start, New_Item => Syllable);
                  Item_Index := Item_Index + 1;
               end loop Fill_Male_Syllables_Start_Loop;
               Item_Index := 0;
               Fill_Male_Syllables_Middle_Loop :
               loop
                  Syllable :=
                    To_Bounded_String
                      (Source =>
                         Value
                           (Item =>
                              Get_Ada_List_Value
                                (L_Index => 5, I_Index => Item_Index)));
                  exit Fill_Male_Syllables_Middle_Loop when Length
                      (Source => Syllable) =
                    0;
                  SyllableString_Container.Append
                    (Container => Male_Syllables_Middle, New_Item => Syllable);
                  Item_Index := Item_Index + 1;
               end loop Fill_Male_Syllables_Middle_Loop;
               Item_Index := 0;
               Fill_Male_Syllables_End_Loop :
               loop
                  Syllable :=
                    To_Bounded_String
                      (Source =>
                         Value
                           (Item =>
                              Get_Ada_List_Value
                                (L_Index => 6, I_Index => Item_Index)));
                  exit Fill_Male_Syllables_End_Loop when Length
                      (Source => Syllable) =
                    0;
                  SyllableString_Container.Append
                    (Container => Male_Syllables_End, New_Item => Syllable);
                  Item_Index := Item_Index + 1;
               end loop Fill_Male_Syllables_End_Loop;
               Item_Index := 0;
               Fill_Male_Vocals_Loop :
               loop
                  Syllable :=
                    To_Bounded_String
                      (Source =>
                  ...
                  Item_Index := Item_Index + 1;
               end loop Fill_Male_Vocals_Loop;
               Item_Index := 0;
               Fill_Male_Consonants_Loop :
               loop
                  Syllable :=
                    To_Bounded_String
                      (Source =>
                         Value
                           (Item =>
                              Get_Ada_List_Value
                                (L_Index => 8, I_Index => Item_Index)));
                  exit Fill_Male_Consonants_Loop when Length
                      (Source => Syllable) =
                    0;
                  SyllableString_Container.Append
