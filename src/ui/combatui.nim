  ...
  window(name = windowName, x = dialogX, y = dialogY, w = width, h = height,
    ...
    imageButton(image = images[unselectAllIcon],
      ...
      if dialog == boardingDialog:
        ...
        for checked in defenders.mitems:
          checked = false
    setLayoutRowDynamic(height = height - (dialogButtonHeight * 2) - 60, cols = 1)
    group(title = "CrewMembersGroup", flags = {windowNoFlags}):
...
proc showPlayerCrewOrders(dialog: var GameDialog; faction: FactionData)
  ...
  ## Returns the modified parameter dialog. It is modified if any error
  ## happened.
  group(title = "Your ship info:", flags = {windowBorder, windowTitle}):
    if dialog != none:
      windowDisable()
    # Show tab buttons
  ...
  group(title = "Enemy info:", flags = {windowBorder, windowTitle}):
    ...
    if enemyTab == 0:
      ...
      for module in game.enemy.ship.modules.mitems:
        ...
        var damagePercent: int = ((module.durability.float /
            module.maxDurability.float) * 100.0).int
        changeStyle(field = progressbar,
          color = (if damagePercent == 100: theme.colors[greenColor]
            elif damagePercent > 24: theme.colors[yellowColor]
            else: theme.colors[redColor])):
