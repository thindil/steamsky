...
proc setAvailableOrders(memberIndex: Natural; ordersBox,
    button: string) {.sideEffect, raises: [], tags: [WriteIOEffect].} =
...
proc getAdaHighestSkill(memberIndex: cint): cstring {.raises: [], tags: [WriteIOEffect], exportc.} =
  try:
...
proc setAdaAvailableOrders(memberIndex: cint; ordersBox,
    button: cstring) {.raises: [], tags: [WriteIOEffect], exportc.} =
  try:
