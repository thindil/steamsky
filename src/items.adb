...
package body Items is
   ...
   begin
      ...
      if Inventory_Sort_Order in DURABILITYASC | DURABILITYDESC then
         Left_Damage :=
           Float(Left.Durability) / Float(Default_Item_Durability);
         Right_Damage :=
           Float(Right.Durability) / Float(Default_Item_Durability);
      ...
      if Inventory_Sort_Order = TYPEASC
        and then
          (if Items_List(Left.ProtoIndex).ShowType /= Null_Unbounded_String
           then Items_List(Left.ProtoIndex).ShowType
           else Items_List(Left.ProtoIndex).IType) <
          (if Items_List(Right.ProtoIndex).ShowType /= Null_Unbounded_String
           then Items_List(Right.ProtoIndex).ShowType
           else Items_List(Right.ProtoIndex).IType) then
         return True;
      end if;
