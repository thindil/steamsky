...
package body Items is
   ...
   begin
      ...
      for I in 0 .. Length(NodesList) - 1 loop
         ...
         Action :=
           (if
              Get_Attribute(ItemNode, "action")'Length > 0
            then
         ...
         if Action /= REMOVE then
            ...
            for J in 0 .. Length(ChildNodes) - 1 loop
               ...
               (New_Item =>
                  Integer'Value(Get_Attribute(Item(ChildNodes, J), "value")));
            ...
            if Action /= UPDATE then
               ...
               Log_Message
                 ("Item updated: " & To_String(TempRecord.Name),
                  EVERYTHING);
            end if;
         else
   ...
   begin
      ...
      if DamageInfo and then Item.Durability < 100 then
         Append
           (ItemName,
            " (" & GetItemDamage(Item.Durability, ToLower) & ")");
      end if;
         ...
         for I in Inventory.Iterate loop
            if Inventory(I).ProtoIndex = ProtoIndex
              ...
              ((Items_List(Inventory(I).ProtoIndex).Value.Length > 0
                and then
                  Items_List(Inventory(I).ProtoIndex).Value(1) <=
                  Quality) or
