...
proc steamsky() {.raises: [], tags: [ReadIOEffect, RootEffect], contractual.} =
  ...
  const
    ...
    showDialog: array[GameDialog.errorDialog..GameDialog.destinationDialog, proc(
      ...
      GameDialog.errorDialog: showError, waitDialog: showWaitMenu,
        ...
        destinationDialog: showDestinationMenu]
  ...
  while true:
    ...
    if redraw:
      ...
      if dialog in GameDialog.errorDialog..GameDialog.destinationDialog:
      ...
      window(name = "Main", x = 0, y = 0, w = windowWidth,
        ...
        if state in showGame.low..showGame.high:
