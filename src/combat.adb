...
package body Combat is
   ...
   -- FUNCTION
   -- Set the new value for Trun_Number
   -- PARAMETERS
   -- New_Value - the new value of the Turn_Number
   -- SOURCE
   procedure Set_Turn_Number(New_Value: Natural) is
      -- ****
   begin
      Turn_Number := New_Value;
   end Set_Turn_Number;
   ...
   procedure Combat_Turn is
      ...
      procedure Attack(Ship, Enemy_Ship: in out Ship_Record) is
         ...
         begin
            ...
            for I in Enemy_Ship.Modules.Iterate loop
               if Get_Module(Index => Enemy_Ship.Modules(I).Proto_Index)
                   .M_Type =
                 M_Type and
                 Enemy_Ship.Modules(I).Durability > 0 then
            ...
            for J in Enemy_Ship.Modules.Iterate loop
               if
                 ((Enemy_Ship.Modules(J).M_Type = TURRET
                  ...
                  Get_Module(Index => Enemy_Ship.Modules(J).Proto_Index)
                      .M_Type =
                    BATTERING_RAM) and
            ...
            if Ship.Modules(K).M_Type in GUN | HARPOON_GUN then
               ...
               if Ammo_Index_2 in
                 ...
                 and then
                   ...
                   Get_Ada_Item_Type
                     (Item_Index =>
                        Get_Module(Index => Ship.Modules(K).Proto_Index)
                          .Value -
                        1) then
   ...
   begin
         ...
         if Enemy.Ship.Modules(I).M_Type in GUN | HARPOON_GUN then
            ...
            if Ammo_Index_2 in
               ...
               if Get_Proto_Item
                 ...
                 Get_Ada_Item_Type
                   (Item_Index =>
                      Get_Module(Index => Enemy.Ship.Modules(I).Proto_Index)
                        .Value -
                      1) then
            ...
            if Enemy_Ammo_Index = 0 then
               ...
               for K in 1 .. Get_Proto_Amount loop
      ...
      if not End_Combat then
         ...
         begin
            ...
            if Was_Boarded and Ship_Free_Space > 0 then
               ...
               for Item of Enemy.Ship.Cargo loop
                  ...
                  if Get_Proto_Item(Index => Item.Proto_Index).Price = 0 and
                  ...
                  if Loot_Amount > 0 then
                     ...
                     Message :=
                       ...
                       To_String
                         (Source =>
                            Get_Proto_Item(Index => Item.Proto_Index).Name);
                     Ship_Free_Space := Free_Cargo(Amount => 0);
         ...
         begin
            if Get_Proto_Ship(Proto_Index => Enemy_Ship_Index).Owner =
