...
proc steamsky() {.raises: [], tags: [ReadIOEffect, RootEffect], contractual.} =
  ...
  const
    ...
    showDialog: array[GameDialog.errorDialog..GameDialog.moduleDialog, proc(
  ...
  while true:
    ...
    if redraw:
      ...
      if dialog in GameDialog.errorDialog..GameDialog.moduleDialog:
