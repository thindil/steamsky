      ...
      if Player_Ship.Speed = DOCKED then
         ...
         if HaveTrader and Sky_Bases(BaseIndex).Population > 0 then
            ...
            for I in Recipes_List.Iterate loop
               if Known_Recipes.Find_Index(Item => To_Unbounded_String(To_String(Source => Recipes_Container.Key(I)))) =
                 UnboundedString_Container.No_Index and
                 Bases_Types_List(Sky_Bases(BaseIndex).Base_Type).Recipes
                   .Contains
                   (To_Unbounded_String(Source => To_String(Source => Recipes_Container.Key(I)))) and
