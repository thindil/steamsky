...
proc sellItems*(itemIndex: Natural; amount: string) {.raises: [
  ...
  body:
    ...
    if playerShip.cargo[itemIndex].durability < getItemMaxDurability(item = playerShip.cargo[itemIndex]):
    ...
    if baseIndex > 0:
      ...
      if not cargoAdded:
        traderCargo.add(y = BaseCargo(protoIndex: protoIndex,
          ...
          maxDurability: getItemMaxDurability(item = playerItem),
...
proc getTradeData*(iIndex: int): tuple[protoIndex, maxSellAmount, maxBuyAmount,
  ...
  if iIndex < 0:
    if baseIndex == 0:
      ...
      result.maxDurability = getItemMaxDurability(item = skyBases[baseIndex].cargo[baseCargoIndex])
  else:
      result.maxDurability = getItemMaxDurability(item = playerShip.cargo[cargoIndex])
  if iIndex < 0:
    if baseIndex == 0:
