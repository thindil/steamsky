...
--  end read only
--  begin read only
   procedure Wrap_Test_UpdateCargo_87d3a7_53988c (Ship: in out ShipRecord; ProtoIndex: Unbounded_String := Null_Unbounded_String; Amount: Integer; Durability: Items_Durability := Default_Item_Durability; CargoIndex, Price: Natural := 0) 
   is
   begin
      begin
         pragma Assert
...
--  id:2.2/87d3a721378c9b6a/UpdateCargo/1/0/test_updatecargo/
   ...
   procedure UpdateCargo (Ship: in out ShipRecord; ProtoIndex: Unbounded_String := Null_Unbounded_String; Amount: Integer; Durability: Items_Durability := Default_Item_Durability; CargoIndex, Price: Natural := 0) renames Wrap_Test_UpdateCargo_87d3a7_53988c;
--  end read only
...
--  begin read only
   function Wrap_Test_GetItemsAmount_df8553_e4797c (IType: String)  return Natural
   is
   begin
      begin
         pragma Assert
           (IType in "Drinks" | "Food");
         null;
      exception
            when System.Assertions.Assert_Failure =>
               AUnit.Assertions.Assert
      ...
      declare
         Test_GetItemsAmount_df8553_e4797c_Result : constant Natural := GNATtest_Generated.GNATtest_Standard.Ships.Cargo.GetItemsAmount (IType);
      begin
         begin
            pragma Assert
              (True);
...
--  begin read only
   ...
   procedure Test_GetItemsAmount_df8553_e4797c (Gnattest_T : in out Test) renames Test_GetItemsAmount_test_getitemsamount;
--  id:2.2/df8553144fad6203/GetItemsAmount/1/0/test_getitemsamount/
   procedure Test_GetItemsAmount_test_getitemsamount (Gnattest_T : in out Test) is
      function GetItemsAmount (IType: String) return Natural renames Wrap_Test_GetItemsAmount_df8553_e4797c;
--  end read only
   ...
   begin
      ...
      Assert
        (GetItemsAmount("Drinks") = 200,
         "Failed to get amount of drinks.");
