--  This package has been generated automatically by GNATtest.
--  Do not edit any part of it, see GNATtest documentation for more details.

--  begin read only
with Gnattest_Generated;

package Bases.Trade.Test_Data.Tests is

   type Test is new GNATtest_Generated.GNATtest_Standard.Bases.Trade.Test_Data
     .Test with
   null record;

   procedure Test_Buy_Recipe_f9dd3c_e2270a(Gnattest_T: in out Test);
   --  bases-trade.ads:63:4:Buy_Recipe:Test_BuyRecipe

   procedure Test_Heal_Wounded_aad41c_6975e9(Gnattest_T: in out Test);
   --  bases-trade.ads:75:4:Heal_Wounded:Test_HealWounded

   procedure Test_Heal_Cost_115ffd_52d375(Gnattest_T: in out Test);
   --  bases-trade.ads:91:4:Heal_Cost:Test_HealCost

   procedure Test_Train_Cost_7284bc_f49c70(Gnattest_T: in out Test);
   --  bases-trade.ads:110:4:Train_Cost:Test_TrainCost

   procedure Test_Train_Skill_30157e_5fedf8(Gnattest_T: in out Test);
   --  bases-trade.ads:129:4:Train_Skill:Test_TrainSkill

end Bases.Trade.Test_Data.Tests;
--  end read only
